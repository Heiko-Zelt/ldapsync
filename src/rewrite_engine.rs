...
impl Action {
    ...
    pub fn apply(&self, entry: &mut SearchEntry) {
        match self {
            Action::Set { attr, values } => match entry.attrs.get_mut(attr) {
            ...
            Action::Add { attr, values } => match entry.attrs.get_mut(attr) {
                Some(mut existing_values) => {
                    Self::insert_all(&mut existing_values, values);
                }
                None => {
            ...
            Action::Remove { attr, values } => match entry.attrs.get_mut(attr) {
                Some(mut existing_values) => {
...
mod test {
    ...
    pub fn test_action_set() {
        let mut entry = SearchEntry {
            ...
            attrs: HashMap::from([
                ...
                (
                    "givenname".to_string(),
                    vec!["Karl".to_string(), "Heinz".to_string()],
                ),
        ...
        let rules = vec![
            Rule {
                actions: vec![Action::Set {
                    attr: "givenname".to_string(),
                    values: vec!["André".to_string(), "Adrian".to_string()],
                }],
            },
            Rule {
                actions: vec![Action::Set {
                    attr: "description".to_string(),
                    values: vec!["A wonderful person".to_string()],
                }],
            },
        ];
    ...
    pub fn test_rules_serialize() {
        let rules = vec![
            ...
            Rule {
                actions: vec![
                    Action::Set {
                        attr: "sn".to_string(),
                        values: vec!["Müller".to_string()],
                    },
                    Action::Clear {
                        attr: "street".to_string(),
                    },
                ],
            },
        ];
        ...
        let result = serde_json::to_string_pretty(&rules).unwrap();
    ...
    pub fn test_rules_deserialize() {
        ...
        let result = Rule::parse_rules(&input).unwrap();

        debug!("result: {:?}", result);
        assert_eq!(result, expected_rules);
    }
}
